...
const Checkout = ({ onBack }) => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const response = await fetch('http://31.97.230.67:5001/api/products/default');
  ...
  const createOrder = async () => {
    try {
      const response = await fetch('http://31.97.230.67:5001/api/orders/create', {
        method: 'POST',
        headers: {
  ...
  const handleRazorpayPayment = async (orderData) => {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          // Verify payment
          const verifyResponse = await fetch('http://31.97.230.67:5001/api/orders/verify-payment', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
